...
int main(int argc, char *argv[]) {
	...
	for (j = 0; j <= partitions_gamma; j++) {
		// Initial conditions
		for (k2 = 0; k2 <= partitions_R; k2++) {
			for (k1 = 0; k1 <= partitions_R; k1++) {
				// Initialize values
				float X[]= {R1_init, R2_init, atof(argv[4]), atof(argv[5]), atof(argv[6]), atof(argv[7])}; // R1.init, R2.init, p.init, s.init, v1.init, v2.init
