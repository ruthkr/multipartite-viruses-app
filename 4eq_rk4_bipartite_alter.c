...
void virus_dynamic(float *, float *, float *, float);
void rk4sys(int, float, float *, float *, int);

void virus_dynamic(float *X, float *f, float *param, float h) {
	f[0] = (param[0] * X[0] * X[2] * (1 - X[0] - X[1]) - param[4] * X[0]) * h;
	f[1] = (param[1] * X[1] * X[2] * (1 - X[0] - X[1]) - param[5] * X[1]) * h;
	f[2] = (param[2] * X[0] * (1 - X[2] - X[3]) - param[6] * X[2]) * h;
	f[3] = (param[3] * X[1] * (1 - X[2] - X[3]) - param[7] * X[3]) * h;
}
...
void rk4sys(int n, float h, float *param, float *X, int nsteps) {
	...
	for (step = 0; step < nsteps; step++) {
		virus_dynamic(X, F[1], param, h);
		for (i = 0; i <= n; i++) {
			Y[i] = X[i] + 0.5 * F[1][i];
		}
		virus_dynamic(Y, F[2], param, h);
		for (i = 0; i <= n; i++){
			Y[i] = X[i] + 0.5 * F[2][i];
		}
		virus_dynamic(Y, F[3], param, h);
		for (i = 0; i <= n; i++){
			Y[i] = X[i] + F[3][i];
		}
		virus_dynamic(Y, F[4], param, h);
		for (i = 0; i <= n; i++){
			X[i] = X[i] + (F[1][i] + 2 * F[2][i] + 2 * F[3][i] + F[4][i])/6;
		}
